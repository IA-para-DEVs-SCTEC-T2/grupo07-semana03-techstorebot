...
| `handlers/message_handler.py` | `POST /message` — valida entrada, chama `MessageHandler`, formata resposta |
...
*Para qualquer* corpo de requisição JSON válido com o campo `"text"` não-vazio, o endpoint `POST /message` SHALL retornar status HTTP 200 e um JSON com o campo `"response"`.
...
*Para qualquer* corpo de requisição ausente, não-JSON ou sem o campo `"text"`, o endpoint `POST /message` SHALL retornar status HTTP 400 e um JSON com o campo `"error"` descritivo.
...
| Corpo de requisição ausente ou malformado | HTTP 400 com mensagem de erro JSON |
...
- Endpoint `POST /message` via `FastAPI TestClient`: smoke test de 200 e 400.
